...
Showcase：在样式设置中选择 **Slide → Slide effect → Duo Fold**。正式播放沿用当前来源的 `PagingPlayItems`、`InfinitePagerController` 与 `PagerItem`，支持已有的本地/网络图片来源、鉴权、缓存、元数据和显示模式。

独立 Demo 仍保留，使用 URL 示例图片，不需要账号或相册配置；首次使用需要网络。仓库和安装包不包含示例照片。
...
默认是带标题、留白和控制面板的控件预览。图片上下各预留 14% 高度，使折叠时的完整轮廓露在背景上；横屏手机采用图片在左、控制面板在右的布局。点击“进入全屏”后图片以 Crop 模式铺满屏幕，不显示按钮，左右滑动翻页；拖动不足图片宽度 20% 时松手会回弹。控件面板新增默认开启的“翻页全局视角”开关：翻页时以中心为支点随折叠角度缩小，最小缩至 74%，为上下透视轮廓留出空间；翻页完成或短滑动回弹时恢复 100%。缩放直接跟随进度，手势区域仍为完整屏幕；关闭开关可恢复原来的全屏裁剪效果。系统返回可退出纯图片模式，回到控件预览。Android 调试入口隐藏状态栏和导航栏，可从屏幕边缘滑动临时唤出系统栏。

拖动滑块可停在任意进度；上一张/下一张播放一次；自动播放循环浏览八张样图；拖动滑块会暂停播放；“渐变模糊”开关用于对比。

## 正式播放与组件复用

正式的 `DuoFoldPager` 预加载当前页两侧的 `PagerItem`，用 GraphicsLayer 引用记录其完整画面（包含 Fit 模式背景），没有额外请求或每帧 CPU 截图。自动播放等待当前页和下一页加载结束，按 Slide 间隔切换；失败或不支持的条目显示现有错误界面后继续播放。手势、桌面方向键、后台暂停、刷新时的内容锚定沿用现有播放基础设施。图片信息和 AI 操作由独立的屏幕覆盖层绘制，不参与折叠。

Duo Fold 使用水平翻页和增强渐变模糊；静止时图片始终以直角铺满窗口。开启翻页后退时，圆角与后退幅度使用同一条曲线，从 0dp 增至最多 24dp，再随恢复全屏归零；关闭后退时始终为直角。自动播放、方向键和翻页按钮的完整动画统一为 2.4 秒，角度进度均匀推进，避免额外的先快后慢缓动让后半程显得停滞；手势松开后的吸附归位保留 900ms 缓动。Slide 设置中的“翻页全局视角”默认开启并持久化，可关闭以对比原尺寸翻折。旧的 Effect 编号保持不变，Duo Fold 使用编号 5。

渲染缓存不再订阅翻折正反面的状态：跨过 50% 时只更换模糊图层的输入引用，复用两张原图的记录和现有 BlurEffect，避免中点重新构建整套缓存。模糊遮罩使用精确的两点线性渐变，替代每级每帧的 33 点采样；模糊强度保持不变。回归测试覆盖中点不重录静态图片、普通连续帧缓存复用和圆角随后退归零。实际 Android GPU 帧耗时仍需在在线设备上验证。

平台能力由 `DuoFoldAvailability` 统一控制：Android 要求 API 31+ 且当前 View 启用硬件加速；桌面和 iOS 使用 Compose 的 Skia 后端，并查询 `BlurEffect.isSupported()`。Web JS/Wasm 暂未验证这组嵌套图层效果，平台开关保守关闭。应用宿主还可通过 `LocalDuoFoldEnabled` 统一关闭功能，不能用它绕过平台能力限制。

不支持时，设置中隐藏 Duo Fold 及其后退开关；读取已有的 effect=5 配置时，设置显示与播放路由都回退到普通 Slide，保留原始配置值，回到支持的平台后仍可使用。直接调用 `DuoFoldPager` 同样受保护，不会先创建折叠图层。
...
```sh
./gradlew :composeApp:desktopTest --tests '*FoldImageTransitionTest' --tests '*DuoFoldPagerTest' \
...
控件预览在开关开启时保留 24dp 圆角；全屏 Demo 与 Showcase 的圆角只随翻页后退出现，恢复全屏时归零。关闭后退时不加圆角。组件可通过 `cornerRadius` 指定最大圆角，默认 0dp，通过 `roundCornersOnlyWhileFolding` 开启随进度变化。动态圆角在绘制阶段计算，不会逐帧使图片和模糊缓存失效；中缝保持连续，也不会裁掉留白中的折叠部分。
